    ...
    <div className="h-full w-full bg-base-200 p-4 flex flex-col">
      <nav className="flex-grow pt-12 flex flex-col space-y-8">
        ...
        >
          <button
            onClick={toggleCart}
            className="text-primary hover:text-neutral text-4xl flex items-center"
          >
            Your Cart
            <svg
              className={`ml-2 h-6 w-6 transform transition-transform ${isCartExpanded ? 'rotate-180' : ''}`}
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
            ...
            {isCartExpanded && (
              <div className="mt-4 space-y-2">
                {displayedItems.map((item, index) => (
                  <div key={index} className="text-sm text-primary ml-4">{item}</div>
                ))}
                {totalPages > 1 && (
        ...
        <Link to="/faq" className="text-primary hover:text-neutral text-2xl">
          Register
        </Link>
        <Link to="/faq" className="text-primary hover:text-neutral text-2xl">
          Login
